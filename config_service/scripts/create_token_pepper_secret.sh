...
SECRET_NAME="${SECRET_NAME:-solidai-sre-config-service/token_pepper}"
...
aws --profile "$AWS_PROFILE" --region "$AWS_REGION" secretsmanager create-secret \
  --name "$SECRET_NAME" \
  --description "solidai-sre-config-service TOKEN_PEPPER (server-side pepper for hashing opaque bearer tokens)" \
