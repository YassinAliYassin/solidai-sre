#!/bin/bash
# Add GitHub App secrets to SolidAI SRE config-service
# Usage: ./add-github-app-secrets.sh
set -e

NAMESPACE="solidai-sre"

echo "🔐 Adding GitHub App secrets to SolidAI SRE"
echo "============================================"
echo ""
...
CURRENT_ADMIN_TOKEN=$(kubectl get secret solidai-sre-config-service -n $NAMESPACE -o jsonpath='{.data.ADMIN_TOKEN}' | base64 -d)
CURRENT_TOKEN_PEPPER=$(kubectl get secret solidai-sre-config-service -n $NAMESPACE -o jsonpath='{.data.TOKEN_PEPPER}' | base64 -d)
CURRENT_IMPERSONATION_JWT_SECRET=$(kubectl get secret solidai-sre-config-service -n $NAMESPACE -o jsonpath='{.data.IMPERSONATION_JWT_SECRET}' | base64 -d)
echo "   ✅ Current values retrieved"

# Step 2: Update secret with GitHub App values
echo ""
echo "2️⃣  Updating secret with GitHub App values..."
kubectl create secret generic solidai-sre-config-service \
  --namespace=$NAMESPACE \
  ...
  --from-literal=GITHUB_APP_NAME="${GITHUB_APP_NAME:-solidai-sre}" \
...
kubectl set env deployment/solidai-sre-config-service -n $NAMESPACE \
  GITHUB_APP_ID- \
...
kubectl set env deployment/solidai-sre-config-service -n $NAMESPACE \
  --from=secret/solidai-sre-config-service \
...
kubectl set env deployment/solidai-sre-config-service -n $NAMESPACE \
  GITHUB_SETUP_REDIRECT_URL="https://ui.solidai-sre.ai/integrations/github/setup"
...
kubectl set env deployment/solidai-sre-orchestrator -n $NAMESPACE \
...
kubectl rollout status deployment/solidai-sre-config-service -n $NAMESPACE --timeout=3m
kubectl rollout status deployment/solidai-sre-orchestrator -n $NAMESPACE --timeout=3m
