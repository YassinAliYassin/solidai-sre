...
export async function GET(request: NextRequest) {
  ...
  try {
    // Get tools catalog from config service
    const res = await fetch(`${CONFIG_SERVICE_URL}/api/v1/team/tools/catalog`, {
      headers: {
    ...
    console.error('Failed to fetch tools catalog:', error);
